...
export function UploadSlots({ defaultValue = [] }) {
  ...
  return <fieldset className="assignment-upload"><legend>Requested files</legend>
    <p>Choose how many files students must upload and name each one. Every file has a 15 MB limit.</p>
    <label>Number of required files<select value={slots.length} onChange={e => {
      const count = Number(e.target.value);
      setSlots(previous => Array.from({ length: count }, (_, i) => previous[i] || { id: `slot-${Date.now()}-${i}`, label: '' }));
    }}>{[0,1,2,3,4,5].map(n => <option key={n} value={n}>{n === 0 ? 'No required files — optional uploads or project link' : n}</option>)}</select></label>
