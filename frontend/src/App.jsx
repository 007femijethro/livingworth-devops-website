...
function submissionLabel(status) {
  if (status === "submitted" || status === "needs_correction") return "Awaiting final result";
...
function LearningCenter({ mode, demo = false }) {
  ...
  async function toggleMaterialVisibility(material) {
    try {
      const data = await api(`/staff/learning/materials/${material.id}/visibility`, { method: "PATCH", body: JSON.stringify({ visible: !material.visible }) });
      setMessage(data.message); load();
    } catch (error) { setMessage(error.message); }
  }
  ...
  if (staff) return (
    <section className="learning-center">
      ...
      {view === "submissions" && <div className="submission-review-list">{submissions.length === 0 ? <div className="empty">No assignment submissions yet.</div> : submissions.map((submission) => <article key={submission.id}><div className="submission-heading"><div><span>{submission.assignmentTitle}</span><h3>{submission.studentName}</h3><p>{submission.email} · Submitted {new Date(submission.submittedAt).toLocaleString()}</p></div><div><b className={`work-status ${submission.status}`}>{submissionLabel(submission.status)}</b>{submission.isLate ? <b className="late-flag">Late</b> : null}</div></div>{submission.submissionUrl && <a href={submission.submissionUrl} target="_blank" rel="noreferrer">Open submitted project ↗</a>}<SubmissionFiles submission={submission} />{submission.note && <p className="student-note">“{submission.note}”</p>}<form onSubmit={(event) => reviewSubmission(event, submission.id)}><label>Final score<input name="score" type="number" min="0" max="1000" defaultValue={submission.score ?? ""} required /></label><label className="review-feedback">Final feedback<textarea name="feedback" rows="3" defaultValue={submission.feedback || ""} required /></label><button className="button primary">Publish final result</button></form></article>)}</div>}
  ...
  return (
    <section className="learning-center student-learning"><div className="learning-head"><div><p className="eyebrow">DevOps programme</p><h2>Learning workspace</h2><p>Open your lessons, complete assignments and track mentor feedback.</p></div><div className="course-progress"><strong>{progress.percentage}%</strong><span>{progress.completed} of {progress.total} assignments completed</span></div></div><div className="progress-track"><span style={{ width: `${progress.percentage}%` }} /></div><p className="form-message success">{message}</p>{modules.length === 0 ? <div className="empty">Your learning modules will appear here when they are published.</div> : <div className="student-modules">{modules.map((module) => <article key={module.id}><div className="student-module-head"><span>Week {module.weekNumber}</span><div><h3>{module.title}</h3><p>{module.summary}</p></div></div>{module.materials.length > 0 && <div className="student-materials"><h4>Learning materials</h4>{module.materials.map((material) => <StudentMaterial key={material.id} material={material} onProgress={updateMaterialProgress} />)}</div>}{module.assignments.map((assignment) => <AssignmentCard key={assignment.id} assignment={assignment} onSubmitted={load} />)}</article>)}</div>}</section>
  );
}
...
function QuizResults({ mode, onLive }) {
  ...
  return <section className="quiz-results">
    ...
    {staff ? <>
      ...
      <details className="quiz-result-section">
      ...
      {results.leaderboard?.length > 0 && <details className="quiz-result-section">
        <summary>{quizId ? "Class leaderboard" : "Combined class leaderboard"}</summary>
        <div className="leaderboard result-leaderboard">{results.leaderboard.map((attempt, index) => <div key={attempt.id}><b>#{index + 1}</b><span>{attempt.studentName}</span><span className="leaderboard-result"><small>{attempt.correctCount}/{attempt.totalQuestions} correct{attempt.combined ? ` · ${attempt.quizCount} quiz${attempt.quizCount === 1 ? "" : "zes"} · ${attempt.attemptCount} attempt${attempt.attemptCount === 1 ? "" : "s"}` : ""} · {Number(attempt.score || 0).toLocaleString()} game points</small><strong>{attempt.percentage}/100</strong></span></div>)}</div>
      </details>}
      {results.topics.length > 0 && <details className="quiz-result-section">
        <summary>Performance by topic</summary>
        <div className="topic-performance">{results.topics.map((topic) => <div key={topic.topic}><span>{topic.topic}</span><div><i style={{ width: `${topic.percentage}%` }} /></div><strong>{topic.percentage}%</strong></div>)}</div>
      </details>}
      {results.hardestQuestions?.length > 0 && <details className="quiz-result-section">
        <summary>Hardest questions</summary>
        <div className="hardest-questions"><p>Unanswered responses count as incorrect.</p>{results.hardestQuestions.map((question, index) => <article key={question.id}><b>#{index + 1}</b><div><strong>{question.prompt}</strong><span>{question.topic} · {question.correct}/{question.attempts} correct · {question.unanswered} unanswered</span></div><i>{question.percentage}%</i></article>)}</div>
      </details>}
      <details className="quiz-result-section">
        <summary>Quiz participation records ({results.attempts.length})</summary>
    ...
    </> : <details className="quiz-result-section student-quiz-history"><summary>Completed quizzes ({results.length})</summary><div className="student-result-list">{results.length === 0 ? <div className="empty">Your completed quizzes will appear here.</div> : results.map((attempt) => <article key={attempt.id}><div><h3>{attempt.title}</h3><p>Attempt {attempt.attemptNo} · {new Date(attempt.completedAt).toLocaleString()}</p></div><span>{attempt.correctCount}/{attempt.totalQuestions} correct</span><strong>{attempt.percentage}/100</strong><button className="profile-button" onClick={() => openResult(attempt.id)}>Review answers</button></article>)}</div></details>}
  </section>;
}
