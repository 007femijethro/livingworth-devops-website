      ...
      {view === "modules" && <div className="learning-staff-grid">
        ...
        <div className="module-admin-list">{modules.length === 0 ? <div className="empty">No modules created yet.</div> : modules.map((module, moduleIndex) => <article key={module.id} className={selectedModule === module.id ? "open" : ""}>
          ...
          {selectedModule === module.id && <div className="module-editor">
            ...
            <form className="combined-material-form" onSubmit={(event) => addMaterial(event, module.id)}><h4>Add learning content</h4><p className="material-help">Complete one or several sections, then add everything together.</p><fieldset><legend>Lesson note</legend><label>Note title<input name="noteTitle" placeholder="e.g. Introduction to Linux" /></label><label>Lesson note<textarea name="lessonContent" rows="8" placeholder="Write or paste the complete lesson note here…" /></label></fieldset><fieldset><legend>YouTube video</legend><label>Video title<input name="videoTitle" placeholder="e.g. Linux explained" /></label><label>YouTube URL<input name="videoUrl" type="url" placeholder="https://youtube.com/watch?v=…" /></label></fieldset><fieldset><legend>Resource link or file</legend><label>Resource title<input name="resourceTitle" placeholder="Optional reading or website" /></label><label>Resource URL<input name="resourceUrl" type="url" placeholder="https://…" /></label><label>File title<input name="fileTitle" placeholder="Optional title for uploaded file" /></label><label>Upload file<input name="file" type="file" accept=".pdf,.doc,.docx,.ppt,.pptx,.txt,.zip" /></label><small>Maximum file size: 10 MB.</small></fieldset><button className="button light-border">Add selected content to this week</button></form>
