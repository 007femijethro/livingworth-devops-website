      ...
      {view === "modules" && <div className="learning-staff-grid">
        ...
        <div className="module-admin-list">{modules.length === 0 ? <div className="empty">No modules created yet.</div> : modules.map((module, moduleIndex) => <article key={module.id} className={selectedModule === module.id ? "open" : ""}>
          ...
          {selectedModule === module.id && <div className="module-editor">
            ...
            <form onSubmit={(event) => addMaterial(event, module.id)}><h4>Add learning material</h4><p className="material-help">Add one item at a time. You can add as many items as this week needs.</p><label>Title<input name="title" required /></label><MaterialFields resourceRequired={false} /><label>Or upload a file<input name="file" type="file" accept=".pdf,.doc,.docx,.ppt,.pptx,.txt,.zip" /></label><small>For a file, leave the resource URL empty. Maximum size: 10 MB.</small><button className="button light-border">Add to this week</button></form>
