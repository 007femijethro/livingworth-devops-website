...
export default function AssignmentsCenter({ staff = false, demo = false }) {
  ...
  return <section className="learning-center assignments-center">
    ...
    {loading ? <p role="status">Loading assignments…</p> : view === 'assignments' ? <>
      ...
      {submissions.filter(s => !status || s.status === status).map(s => <article className="assignment-card" key={s.id}><h3>{s.assignmentTitle}</h3><p><strong>{s.studentName}</strong> · {s.email}</p><p>Submitted {new Date(s.submittedAt).toLocaleString()}{s.isLate ? ' · Late' : ''} · {s.status.replaceAll('_', ' ')}</p>{s.submissionUrl && <a href={s.submissionUrl} target="_blank" rel="noreferrer">Open project ↗</a>}{s.fileName && <AttachmentDownload id={s.id} name={s.fileName} />}{s.note && <p style={{ whiteSpace: 'pre-wrap' }}>{s.note}</p>}<form className="assignment-submit-form" onSubmit={e => save(e, `/staff/submissions/${s.id}/review`, 'PATCH')}><label>Review result<select name="status" defaultValue={s.status === 'completed' ? 'completed' : 'needs_correction'}><option value="needs_correction">Request changes</option><option value="completed">Mark complete</option></select></label><label>Score<input name="score" type="number" min="0" max={assignments.find(a => a.id === s.assignmentId)?.maxScore || 1000} defaultValue={s.score ?? ''} /></label><label>Feedback<textarea name="feedback" rows="3" defaultValue={s.feedback || ''} required /></label><button className="button primary" disabled={busy || demo}>{busy ? 'Saving…' : 'Save review'}</button></form></article>)}
    </>}
  </section>;
}
